...
if [[ "$GITHUB_WORKFLOW" == "Pulp-Docs changelog update" ]]; then
  # Do not build bindings docs on changelog update
  exit
fi

mkdir -p ../pulp-docs-bindings
tar -xvf pulp-docs-python-client-docs.tar --directory ../pulp-docs-bindings
pushd ../pulp-docs-bindings

# publish to docs.pulpproject.org/pulp_docs_client
rsync -avzh site/ doc_builder_pulp_docs@docs.pulpproject.org:/var/www/docs.pulpproject.org/pulp_docs_client/

# publish to docs.pulpproject.org/pulp_docs_client/en/{release}
rsync -avzh site/ doc_builder_pulp_docs@docs.pulpproject.org:/var/www/docs.pulpproject.org/pulp_docs_client/en/"$2"
popd
